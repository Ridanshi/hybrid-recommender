...
def _build_test_data(data_path: str | None = None):
    ...
    # Item ratings are intentionally excluded from this criterion.
    # Mixing a global "rating >= threshold" filter into the relevance set
    # would mark unrelated high-rated items as ground-truth matches for
    # every query, contaminating Precision@K, Recall@K, NDCG@K, and MRR
    # regardless of semantic similarity.  Category membership is the sole
    # signal for item-similarity evaluation.
    test_pairs = []
