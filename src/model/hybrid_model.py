...
class HybridRecommender:
    ...
    def _normalize_scores(self, scores):
        ...
        1. **No-information vector** — every value is exactly ``0.0``.
           This arises, for example, when the collaborative model is absent
           and every candidate receives ``raw_collab = 0.0``.  There is no
           signal at all; converting this to ``0.5`` would inject a phantom
           contribution equal to ``weight × 0.5`` into every hybrid score.
           The correct output is ``[0.0, …, 0.0]``.

        2. **Legitimate constant-score vector** — every value is the same
           finite non-zero number (a genuine tie).  The relative ordering
           cannot be determined from the scores alone, so ``0.5`` (the
           midpoint of [0, 1]) is a reasonable and stable choice.

        This distinction is critical for hybrid scoring: collaborative or
        sentiment scores must remain zero when there is genuinely no evidence,
        so that the corresponding weight does not add spurious signal.
        """
        if not scores:
            return scores
        ...
        if self.normalization == 'zscore':
            ...
            if sigma == 0 or math.isnan(sigma):
                # All values identical.  Preserve the "no signal" semantic for
                # all-zero vectors; use 0.5 for genuine constant-score ties.
        ...
        if mx - mn == 0 or math.isnan(mn) or math.isnan(mx):
            # All values identical (or NaN).  Same two-case logic as above.
